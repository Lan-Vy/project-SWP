...
//      FIX BEGIN HERE
        // Use to store quantity and price for each size
        List<String> detailQuantityAndSize = new ArrayList<>();

        // Loop through products to get quantity and price for each size
        for (int i = 0; i < listP.size(); i++) {
            // Get quantity and price information for each size of the product
            String quantityAndPriceBySize = dao.getQuantityAndPriceBySize(listP.get(i).getId());
            detailQuantityAndSize.add(quantityAndPriceBySize);
        }
        request.setAttribute("detailQuantityAndSize", detailQuantityAndSize);
// DONE FIXED
