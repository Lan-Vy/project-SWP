...
public class ProductDetailControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String cartSuccess = (String) request.getSession().getAttribute("cartSuccess");

        if (cartSuccess != null) {
            request.setAttribute("cartSuccess", cartSuccess);
            request.getSession().removeAttribute("cartSuccess"); // Xoá sau khi dùng
        }
        String cartFail = (String) request.getSession().getAttribute("cartFail");
        if (cartFail != null) {
            request.setAttribute("cartFail", cartFail);
            request.getSession().removeAttribute("cartFail"); // Xoá sau khi dùng
        }
        ...
        Product pSize = dao.getAmoutSizeOfEachSize(Integer.parseInt(pID), Integer.parseInt(sizeId));
        int cid = p.getCateID();
        ...
        List<Feedback> feedbacks = fdao.getFeedbacksByProductId(productID);
        request.setAttribute("feedbacks", feedbacks);
        
        double avgRating = fdao.getAverageRatingByProductId(productID);
